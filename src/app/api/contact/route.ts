...
// Memastikan route ini dinamis (tidak di-prerender saat build time)
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 2. Cek apakah API Key Resend tersedia di environment
    ...
    // 3. Inisialisasi Resend hanya saat request POST dieksekusi
    const resend = new Resend(apiKey);
    const toEmail = process.env.CONTACT_TO_EMAIL || "pt.ahlulkaryautama@gmail.com";
    const fromEmail = process.env.CONTACT_FROM_EMAIL || "onboarding@resend.dev";

    // 4. Kirim notifikasi ke email pemilik portfolio
    const notifyResult = await resend.emails.send({
      from: `Portfolio Contact <${fromEmail}>`,
      to: toEmail,
      replyTo: email,
      subject: `[Portfolio Inquiry] New ${scope || "General"} project from ${name}`,
      html: `
        <div style="font-family: monospace; background: #0A0A0A; color: #F7F4EF; padding: 32px; border-radius: 12px; max-width: 600px;">
          ...
          <table style="width: 100%; border-collapse: collapse;">
            ...
            <tr><td style="padding: 8px 0; color: #888;">Scope</td><td style="color: #F7F4EF;">${scope || "General"}</td></tr>
          </table>
          <hr style="border-color: #1C1C1E; margin: 24px 0;" />
    ...
    // 5. Kirim email konfirmasi otomatis ke client (opsional jika domain sudah diverifikasi)
    try {
        ...
        html: `
          <div style="font-family: monospace; background: #0A0A0A; color: #F7F4EF; padding: 32px; border-radius: 12px; max-width: 600px;">
            ...
            <p style="color: #888; font-size: 13px; line-height: 1.7;">
              Thank you for reaching out. I've received your project inquiry for <strong style="color: #F7F4EF;">${scope || "your project"}</strong> and will review your specifications.
            </p>
            <p style="color: #888; font-size: 13px; line-height: 1.7;">
      ...
      // Free tier Resend with onboarding@resend.dev only permits sending to registered user email
      console.warn("Auto-reply skipped (requires custom verified domain in Resend):", autoReplyErr);
    }
